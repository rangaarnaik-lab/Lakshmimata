...
// ── Vol climax / near EMA / IBV (OR) ──────────────────────────────────
// Matches if ANY of these is true: recent HY / HT / Bull Snort (last
// 10 days), or price near EMA9, EMA21, IBV, or a HY/HT day today.
function calcVolClimaxNearSupport(s, lookback=10){
  const hyHist = s.hy?.history || []
  const htHist = s.ht?.history || []
  const recentHY = !!(s.hy?.isHY) || hyHist.slice(-lookback).some(Boolean)
  const recentHT = !!(s.ht?.isHT) || htHist.slice(-lookback).some(Boolean)
  const recentSnort = !!s.isBullSnort
  const hadRecentClimax = recentHY || recentHT || recentSnort

  const nearEma9 = !!s.nearEMA9?.isNearEMA9
  const nearEma21 = !!s.nearEMA21?.isNearEMA21
  const nearIbv = !!calcIBV(s).isIBV
  const nearHyHt = !!(s.hy?.isHY) || !!(s.ht?.isHT)
  const nearSupport = nearEma9 || nearEma21 || nearIbv || nearHyHt

  // OR across climax + near-support (not AND)
  const isMatch = hadRecentClimax || nearSupport
  const tags = []
  if(recentHY) tags.push('HY')
  if(recentHT) tags.push('HT')
  if(recentSnort) tags.push('Snort')
  const near = []
  if(nearEma9) near.push('EMA9')
  if(nearEma21) near.push('EMA21')
  if(nearIbv) near.push('IBV')
  if(nearHyHt && !tags.includes('HY') && !tags.includes('HT')) near.push(s.ht?.isHT?'HT':'HY')

  const parts = [...tags, ...near]
  return {
    isMatch,
    hadRecentClimax,
    nearSupport,
    recentHY, recentHT, recentSnort,
    nearEma9, nearEma21, nearIbv, nearHyHt,
    label: isMatch ? `🔥⚡ ${parts.join('/')}` : null,
    desc: isMatch
      ? parts.join(' / ')
      : 'No HY/HT/Snort or EMA/IBV near signal',
  }
}
...
const PRESETS = [
  ...
  {id:'volpull',   label:'Vol/EMA',      icon:'🔥', desc:'HY or HT or Bull Snort (recent) OR near EMA9 or EMA21 or IBV or HY/HT'},
...
const SIGNAL_TOOLTIPS = {
  ...
  volpull: 'Matches if ANY is true: recent HY, HT, or Bull Snort (last 10 days), or price near EMA9, EMA21, IBV, or a HY/HT day — OR logic, not AND.',
...
const SIGNAL_GLOSSARY = [
  ...
  ['🔥→⚡ Vol/EMA', 'Matches if ANY is true: recent HY, HT, or Bull Snort (within ~10 days), or price near EMA9, EMA21, IBV, or HY/HT today — OR across all of these, not AND.'],
...
function persistDockLayout(layout){
  try{ localStorage.setItem(DOCK_LAYOUT_KEY, JSON.stringify(normalizeDockLayout(layout))) }catch{/* ignore */}
}
function moveDockTile(order, id, dir){
  const next = [...order]
...
/** Operating margin % — prefer stored opm_pct, else derive from PBT − other income. */
function effectiveOpm(row){
  if(!row) return null
  if(row.opm_pct != null) return row.opm_pct
  return (row.sales && row.pbt != null && row.other_income != null && row.sales !== 0)
    ? (row.pbt - row.other_income) / row.sales * 100 : null
}
 ...
 *
 * Margin + other-income caps are tracked as maxTierIdx so a later QoQ
 * "recovery" bump cannot undo them (APOLLO-style: PAT/Sales YoY strong
 * but OPM −4pp and other income spiked — not Excellent).
 */
function computeResultRating(hist){
  ...
  const curOpm = effectiveOpm(current)
  const yoyOpm = effectiveOpm(yoyRow)
  const opmYoy = pct(curOpm, yoyOpm)
  const opmPp = (curOpm!=null && yoyOpm!=null) ? (curOpm - yoyOpm) : null
  const otherIncomeSpike = (current.other_income!=null && prevQtr?.other_income!=null &&
    prevQtr.other_income>0.5 && current.other_income > prevQtr.other_income*1.5)
  ...
  // Ceiling on how good the badge can get — QoQ bumps cannot exceed this.
  let maxTierIdx = tiers.length - 1

  // Headline PAT gate — reported profit is what the market prints.
  if (rawPatYoy!=null && rawPatYoy < 0) {
    maxTierIdx = rawPatYoy <= -15 ? 0 : Math.min(maxTierIdx, 1)
  }

  // Operating-margin gates (same OPM definition as the Results table).
  const softCompress = (opmYoy!=null && opmYoy <= -5) || (opmPp!=null && opmPp <= -1)
  const hardCollapse = (opmYoy!=null && opmYoy <= -20) || (opmPp!=null && opmPp <= -3)
  if (hardCollapse) {
    maxTierIdx = Math.min(maxTierIdx, 1) // Neutral max
    if (opmYoy!=null && opmYoy <= -35 && (rawPatYoy==null || rawPatYoy < 0)) {
      maxTierIdx = Math.min(maxTierIdx, 0)
    }
  } else if (softCompress) {
    maxTierIdx = Math.min(maxTierIdx, 2) // Good max — not Excellent
  }

  // Other-income spike + margin down → profit growth may not be operating.
  if (otherIncomeSpike) {
    const marginDown = (opmPp!=null && opmPp < 0) || (opmYoy!=null && opmYoy < 0)
    maxTierIdx = Math.min(maxTierIdx, marginDown ? 1 : 2) // Neutral or Good max
  }

  if (!resultRating) return null
  let idx = Math.min(tiers.indexOf(resultRating), maxTierIdx)

  // QoQ tier-shift: sequential trend can nudge within the quality ceiling.
  if (prevQtr) {
    ...
    if (patQoq!=null) {
      ...
      const sequentiallyFlat = (salesQoq==null || salesQoq <= 2) && patQoq <= 2
      if (decelerating && idx > 0) idx -= 1
      else if (recovering && idx < maxTierIdx) idx += 1
      if (sequentiallyFlat) idx = Math.min(idx, 2)
    }
  }
...
export default function App(){
  ...
  const applyDockLayout=(next)=>{
    const normalized=normalizeDockLayout(next)
    setDockLayout(normalized)
      ...
      <ScreenerFrame
        ...
        dockStyle={dockSoloChart?{
          ...
          flex:(chartSym&&!isMobile&&!panelWins.screener.float&&(
            ...
            :1,
          height: dockStack ? undefined : '100%',
          minHeight: dockStack ? 180 : 0,
          borderRight: dockStack ? 'none' : undefined,
          borderBottom: dockStack ? `1px solid ${C.divider}` : undefined,
        }}
      >
            ...
            {stocks.length===0?(
              ...
              const GROUPS = [
                {id:'breakouts', group:'📈 Breakouts', items:[
                  ...
                  {key:'volpull',  label:'Vol / EMA / IBV (OR)', color:'#f59e0b', filter:s=>calcVolClimaxNearSupport(s).isMatch},
