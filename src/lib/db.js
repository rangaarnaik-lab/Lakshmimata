...
export async function fetchStocksFromDB({ indexFilter = 'all', watchlistSyms = null, historyDate = null } = {}) {
  ...
  return (data || []).map(row => ({
    ...
    sqMomentum:    row.sq_momentum || 0,
    sqMomentumDir: row.sq_momentum_dir || 'flat',
    sqStrength:    row.sq_strength || 0,
    sqFiredBullish:row.sq_fired_bullish || false,
    sqFiredBearish:row.sq_fired_bearish || false,
    sqDotsD:       typeof row.sq_dots_d==='string'?JSON.parse(row.sq_dots_d||'[]'):(row.sq_dots_d||[]),
    sqHistD:       typeof row.sq_hist_d==='string'?JSON.parse(row.sq_hist_d||'[]'):(row.sq_hist_d||[]),
    sqWeeklyIn:    row.sq_weekly_in || false,
    sqWeeklyFired: row.sq_weekly_fired || false,
    sqWeeklyDays:  row.sq_weekly_days || 0,
    sqWeeklyMom:   row.sq_weekly_mom || 0,
    sqWeeklyMomDir:row.sq_weekly_mom_dir || 'flat',
    sqWeeklyBullish:row.sq_weekly_bullish || false,
    sqHourlyIn:    row.sq_hourly_in || false,
    sqHourlyFired: row.sq_hourly_fired || false,
    sqHourlyDays:  row.sq_hourly_days || 0,
    sqHourlyMom:   row.sq_hourly_mom || 0,
    sqHourlyMomDir:row.sq_hourly_mom_dir || 'flat',
    sqHourlyBullish:row.sq_hourly_bullish || false,
